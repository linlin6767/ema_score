...
#    ks,iv,mbr,a = ks_iv.ks_iv_evaluate(y_test)        
    ...
    return feature,y_test

def model_rocauc(y_testpre,r,y_pre):      
    print "------model evaluate------"
    print metrics.roc_auc_score(y_testpre['is_lend'], y_testpre[y_pre])   
    precision, recall, thresholds =metrics.precision_recall_curve(y_testpre['is_lend'],y_testpre[y_pre]) 
    answer = y_testpre[y_pre]>r
    print metrics.classification_report(y_testpre['is_lend'],answer,target_names=['refuse','lend']) 
    print metrics.confusion_matrix(y_testpre['is_lend'],answer)
    print "Test Accuracy : %.7g" % metrics.accuracy_score(y_testpre['is_lend'], answer)        

if(__name__=='__main__'):
    path = 'E:/fengchao_data/haf_dataans/feature&py/ali_data/'
    ali_data1  =  pd.read_csv(path+'/alifeaget1.csv',sep='\t')
    ali_columns = pd.read_csv(path+'ali_fea/ali_sample.csv')
    a = list(ali_columns.columns)
    a.insert(271,'tua.apply_id')
    ali_data1.columns = a    
    ##数据清洗##    
    ali_data1.rename(columns = lambda x:x.replace('ali.',''), inplace = True) #修改列明
    ali_data = ali_data1.drop_duplicates(['limit_id']) 
...
#------- GBDT -------------
    ...
    model_rocauc(ytest_pre,0.16,'y_pre') #准确率0.8572  auc0.702 
    model_rocauc(ytest_pre,0.88,'rf_rank1') #准确率0.83  auc0.702 
# ------ RF ----------
    ...
    rf_bestparams , rf_bestscore , rf_gridscores = gridsearch(rf,rf_params,x1,y1['is_lend'],0) 
    print rf_bestparams , rf_bestscore , rf_gridscores ##0.682    
...
##将数据分成交易类和资金类分别建模
    ...
    params = {'n_estimators':100, 'max_depth':4,'loss':'ls'}
    gb  = GradientBoostingRegressor(**params)  
    feature3_1,ytest_pre3_1 = fea_split(x3_1,y1,gb,False)
    
    feature3_2,ytest_pre3_2 = fea_split(x3_2,y1,gb,False)
    feature3_3,ytest_pre3_3 = fea_split(x3_3,y1,gb,False)
   ...
   ##模型融合方案1
    ...
    model_rocauc(ytest_pre3_1,0.88,'rf_rank3') #准确率0.83  auc0.6857    
   ##模型融合方案2
    ...
    x2_train = x_train[x_train.columns[rlr.get_support()]].as_matrix()   
    lr.fit(x2_train,y_train)
    print(u'模型的平均正确率为：%s' % lr.score(x2_train,y_train))
    ali_lr2_tr = pd.DataFrame(x2_train)
    ali_lr2_tr['y_train'] = y_train
    ali_lr2_tr['lr2tr_pre']= lr.predict(x2_train)
    lr2_log_pre = pd.DataFrame(lr.predict_log_proba(x2_train))
    ali_lr2_tr['log_pre0'],ali_lr2_tr['log_pre1'] = lr2_log_pre[0],lr2_log_pre[1]
    ali_lr2_tr.to_csv(path+'ali_lr2tr.csv')
